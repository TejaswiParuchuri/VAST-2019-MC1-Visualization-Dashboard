...
for(var i in store_val)
{
  ...
  if(store_val[i][0] == location)
  {
    ...
    list_val = list_val.concat({"x1":x_axis_value,"y1":y_val,"location":location})
    if(max_y < y_val)
    {
      max_y = y_val
    }
  ...
  for(var i in store_val)
{
  ...
  if(store_val[i][0] == location)
  {
    ...
    list_val = list_val.concat({"x1":x_list_value,"y1":y_val,"location":location})
    if(max_y < y_val)
    {
      max_y = y_val
    }
...
function call_bar(list_val){
  ...
  .data(list_val,d=>d.location)
    .join("rect")
      .attr("class", "bar")
      .attr("height", function(d){return heightBar - y_barScale(d.y1)  } )
      .attr("width", x_barScale.bandwidth())
      .attr("x", function(d) {  return x_barScale(d.x1); })
      .attr("y", function(d) { return y_barScale(d.y1); })
}
