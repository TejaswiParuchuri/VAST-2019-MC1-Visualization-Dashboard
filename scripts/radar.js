...
//    height += 348;
    ...
    radarSvg = d3.select("#radar").append('svg').attr("width", width + margin.left + margin.right)
...
function drawRadarChart(arr_len,req_date,req_loc) {
	...
	for (i=0;i<arr_len;i++)
	{
		...
		if(date.getDate()==req_date && date.getHours()>=time_start && date.getHours()<=time_end && reportsData[i]['location']==req_loc)
		{
...
var RadarChart = {
		  draw: function(id, d, options){
		    ...
		    d.forEach(function(y, x){
		      ...
		      .data(y, function(j, i){
				  //console.log(i)
		        dataValues.push([
		        cfg.w/2*(1-(parseFloat(Math.max(j.value*10, 0))/cfg.maxValue)*cfg.factor*Math.sin(i*cfg.radians/total)),
		        cfg.h/2*(1-(parseFloat(Math.max(j.value*10, 0))/cfg.maxValue)*cfg.factor*Math.cos(i*cfg.radians/total))
		        ]);
		...
		var tooltip = d3.select("body").append("div").attr("class", "toolTip");
		    d.forEach(function(y, x){
		      ...
		      return cfg.w/2*(1-(Math.max(j.value*10, 0)/cfg.maxValue)*cfg.factor*Math.sin(i*cfg.radians/total));
		      })
		      .attr("cy", function(j, i){
		        return cfg.h/2*(1-(Math.max(j.value*10, 0)/cfg.maxValue)*cfg.factor*Math.cos(i*cfg.radians/total));
		      })
		        ...
		        //console.log(d.area)
		            tooltip
		              ...
		              .style("display", "inline-block")
		      				.html((d.area) + "<br><span>" + parseFloat(d.value).toFixed(2) + "</span>");
		            })
			...
			circle.on('mouseover', function (d){
						tooltip
						  ...
						  .style("top", d3.event.pageY - 80 + "px")
						  .style("display", "inline-block")
								.html("Shake Intensity: <br><span>" + parseFloat(shake_intensity_radius).toFixed(2) + "</span>");
		            })
					.on("mouseout", function(d){ tooltip.style("display", "none");});

			g.attr("opacity",0).transition().delay(500).transition().attr("opacity",1);
		  }
		};
